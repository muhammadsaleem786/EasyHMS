    ...
    public class emr_patientController : ApiController, IERPAPIInterface<emr_patient_mf>, IDisposable
    {
        ...
        public async Task<ResponseInfo> searchByName(string term)
        {
            ...
            try
            {
                ...
                var PatientInfo = _service.Queryable().Where(a => a.CompanyId == CompanyID && (a.PatientName.Contains(term) || a.CNIC.Contains(term)
                || (a.Mobile).Contains(term))).Select(z => new
                {
                    value = z.ID,
