...
//{
//    [JwtAuthentication]
//    public class pr_leave_typeController : ApiController, IERPAPIInterface<pr_leave_type>, IDisposable
//    {
//        private readonly Ipr_leave_typeService _service;
//        private readonly IUnitOfWorkAsync _unitOfWorkAsync;

//        public pr_leave_typeController(IUnitOfWorkAsync unitOfWorkAsync, Ipr_leave_typeService Service)
//        {
//            _unitOfWorkAsync = unitOfWorkAsync;
//            _service = Service;
//        }

//        public async Task<ResponseInfo> Save(pr_leave_type Model)
//        {
...
//                pr_leave_type LeaveType = _service.Queryable().Where(x => x.CompanyID == CompanyID && x.TypeName.ToLower() == Model.TypeName.ToLower() && x.Category == Model.Category).FirstOrDefault();
//                if (LeaveType == null)
//                {
...
//                    Model.AccuralDropDownID = (int)sys_dropdown_mfEnum.AccrualFrequency;
...
//                {
//                    objResponse.IsSuccess = false;
//                    objResponse.ErrorMessage = Model.TypeName + " already exist. Please enter a different name and try again.";
//                }
...
//        public async Task<ResponseInfo> SaveAndReturnLeaveTypeList(pr_leave_type Model)
//        {
//            var objResponse = new ResponseInfo();
//            try
//            {
...
//                        {
//                            ID = Model.ID,
//                            LeaveTypesList = _service.Queryable().Where(x => x.CompanyID == CompanyID).Select(x => new { ID = x.ID, x.Category, TypeName = x.TypeName, EarnedValue = x.EarnedValue }).ToList(),
//                        };
//                    }
//                    catch (DbUpdateException)
//                    {
...
//        [HttpGet]
//        [ActionName("Update")]
//        public async Task<ResponseInfo> Update(pr_leave_type Model)
//        {
...
//                decimal CompanyID = Request.CompanyID();
//                pr_leave_type leaveType = _service.Queryable().Where(x => x.CompanyID == CompanyID && x.ID != Model.ID && x.TypeName.ToLower() == Model.TypeName.ToLower() && x.Category == Model.Category).FirstOrDefault();
//                if (leaveType == null)
//                {
//                    Model.ModifiedBy = Request.LoginID();
//                    Model.ModifiedDate = Request.DateTimes();
...
//                List<pr_leave_type> Models = _service.Queryable().Where(e => e.CompanyID == CompanyID && IdList.Contains(e.ID)).ToList();
