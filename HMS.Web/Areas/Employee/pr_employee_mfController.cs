    ...
    public class pr_employee_mfController : ApiController, IERPAPIInterface<pr_employee_mf>, IDisposable
    {
        ...
        public ResponseInfo FormLoad()
        {
            ...
            try
            {
                ...
                var DocumentTypes = AllDropdownsData.Where(e => e.DropDownID == (int)sys_dropdown_mfEnum.DocumentType).Select(x => new { x.ID, x.Value }).ToList();
                ...
                objResponse.ResultSet = new
                {
                    ...
                    ContributionList = DefaultAllowancesList,// ContributionList,
                    DeductionList = DefaultAllowancesList,
                    Statuses = Statuses,
                    VacationAndSickLeaves = DefaultAllowancesList,
       ...
       .FirstOrDefault();
        ...
        public ResponseInfo GetById(string Id)
        {
            ...
            try
            {
                ...
                //   var ContractEmp = _service.Queryable()
                //.Where(e => e.CompanyID == CompanyID && e.TypeEndDate <= date && e.EmployeeTypeID != 1)
                //.Select(x => new { x.ID, Name = x.FirstName + " " + x.LastName, ContractStartDate = x.TypeStartDate, ContractEndtDate = x.TypeEndDate }).ToList();
                //   var ContractExpiredEmp = ContractEmp.Where(x => x.ContractEndtDate < dateNow).ToList();
                //   var ContractNeartoExpireEmp = ContractEmp.Where(x => x.ContractEndtDate >= dateNow && x.ContractEndtDate <= date).ToList();

                //   var BirthDays = _service.Queryable()
                //.Where(e => e.CompanyID == CompanyID && e.DOB != null && (e.DOB.Value.Month >= dateNow.Date.Month && e.DOB.Value.Day >= dateNow.Date.Day) && (e.DOB.Value.Month <= date.Date.Month && e.DOB.Value.Day <= date.Date.Day))
                //.Select(x => new { x.ID, Name = x.FirstName + " " + x.LastName, DOB = x.DOB, DaysLeft = (x.DOB.Value.Day - dateNow.Day) }).ToList();

                //   var TodayBirthday = BirthDays.Where(x => x.DOB.Value.Month == dateNow.Date.Month && x.DOB.Value.Day == dateNow.Date.Day).ToList();
                //   var UpComingBirthday = BirthDays.Where(x => x.DOB.Value.Month == dateNow.Date.Month && x.DOB.Value.Day > dateNow.Date.Day).ToList();
