            ...
            try
            {
                ...
                if (IgnorePaging)
                {
                    var DataList = obj.AsEnumerable().Select(m => new
                    {
                        ...
                        Amount = emr_patient_bill.Where(a => a.PatientId == m.PatientId && a.DoctorId == m.DoctorId && a.AppointmentId == m.ID && a.BillDate.Day == m.AppointmentDate.Value.Day && a.BillDate.Month == m.AppointmentDate.Value.Month && a.BillDate.Year == m.AppointmentDate.Value.Year).FirstOrDefault() == null ? 0 : emr_patient_bill.Where(a => a.PatientId == m.PatientId && a.DoctorId == m.DoctorId && a.AppointmentId==m.ID && a.BillDate.Day == m.AppointmentDate.Value.Day && a.BillDate.Month == m.AppointmentDate.Value.Month && a.BillDate.Year == m.AppointmentDate.Value.Year).Sum(z => z.PaidAmount),
                        OutStandingAmount = emr_patient_bill.Where(a => a.PatientId == m.PatientId && a.DoctorId == m.DoctorId && a.AppointmentId == m.ID && a.BillDate.Day == m.AppointmentDate.Value.Day && a.BillDate.Month == m.AppointmentDate.Value.Month && a.BillDate.Year == m.AppointmentDate.Value.Year).FirstOrDefault() == null ? 0 : emr_patient_bill.Where(a => a.PatientId == m.PatientId && a.DoctorId == m.DoctorId && a.AppointmentId == m.ID && a.BillDate.Day == m.AppointmentDate.Value.Day && a.BillDate.Month == m.AppointmentDate.Value.Month && a.BillDate.Year == m.AppointmentDate.Value.Year).Sum(z => z.OutstandingBalance),
                    ...
                    var PreviousPatientList = DataList.Where(x => x.AppointmentDate.Value.Date <= DateTime.Now.Date).ToList();
                ...
                else
                {
                    ...
                    var DataList = obj.AsEnumerable().Select(m => new
                    {
                        ...
                        PatientId = m.PatientId,
                        CompanyId = m.CompanyId,
                        Amount = emr_patient_bill.Where(a => a.PatientId == m.PatientId && a.BillDate.Day == m.AppointmentDate.Value.Day && a.BillDate.Month == m.AppointmentDate.Value.Month && a.BillDate.Year == m.AppointmentDate.Value.Year).FirstOrDefault() == null ? 0 : emr_patient_bill.Where(a => a.PatientId == m.PatientId  && a.BillDate.Day == m.AppointmentDate.Value.Day && a.BillDate.Month == m.AppointmentDate.Value.Month && a.BillDate.Year == m.AppointmentDate.Value.Year).Sum(z => z.PaidAmount),
                    ...
                    var PreviousPatientList = DataList.Where(x => x.AppointmentDate.Value.Date <= DateTime.Now.Date).ToList();
                    var FuturePatientList = DataList.Where(x => x.AppointmentDate.Value.Date >= DateTime.Now.Date).ToList();
