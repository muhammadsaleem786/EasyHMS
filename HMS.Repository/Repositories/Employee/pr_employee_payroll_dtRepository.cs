...
{
    public static class pr_employee_payroll_dtRepository
    {
        public static PaginationResult Pagination(this IRepository<pr_employee_payroll_dt> repository, decimal CompanyID, int CurrentPageNo, int RecordPerPage, string VisibleColumnInfo, string SortName, string SortOrder, string SearchText, bool IgnorePaging = false)
        {
            var objResult = new PaginationResult();
            try
            {
                var PFilter = Utility.SetPaginationFilter(CurrentPageNo, RecordPerPage, VisibleColumnInfo, SortName, SortOrder, SearchText);
                Expression<Func<pr_employee_payroll_dt, bool>> predicate = (e => e.CompanyID == CompanyID);
                ...
                IQueryable<pr_employee_payroll_dt> filteredData = repository.Queryable().Where(predicate);
